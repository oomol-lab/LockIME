...
- **全局锁定，或仅切换**——给全局默认输入源设定三种行为之一：*锁定*，在所有地方把它钉住；*切换*，在你进入任何没有自身规则的应用时为你切换一次，随后便放手让你自由（每进入一个新的无规则应用都会再触发一次）；或**无**，什么都不固定，将 LockIME 当作纯粹的按应用 / 按站点切换器来用。*切换*会在每个无规则应用中主动为你切入；**无**在全局层面什么都不做。
