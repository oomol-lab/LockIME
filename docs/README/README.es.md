...
- **Bloquear de forma global, o solo cambiar** — asigna a la fuente de entrada predeterminada global uno de tres comportamientos: *bloquear* para fijarla en todas partes; *cambiar* para cambiarte a ella una vez al entrar en cualquier aplicación sin regla propia y luego dejarte libre (volviendo a dispararse en cada nueva aplicación sin regla); o **Ninguna** para no fijar nada en absoluto y actuar como un cambiador puro por aplicación/por sitio. *Cambiar* te cambia activamente en cada aplicación sin regla; **Ninguna** no hace nada de forma global.
