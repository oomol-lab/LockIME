...
- **グローバルにロック、または切り替えだけ**——グローバルのデフォルト入力ソースに、3 つの挙動のいずれかを与えられます：*ロック*はどこでもそれを固定します。*切り替え*は、独自のルールを持たないアプリに入ったときに一度だけそれへ切り替えてから自由にさせます（ルールのない新しいアプリごとに再発火）。**なし**は何も固定せず、純粋なアプリごと / サイトごとの切り替え役として振る舞います。*切り替え*はルールのないアプリごとに能動的に切り替え、**なし**はグローバルには何もしません。
