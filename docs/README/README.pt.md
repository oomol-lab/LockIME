...
- **Bloquear globalmente, ou apenas alternar** — dê à fonte de entrada padrão global um de três comportamentos: *bloquear* para fixá-la em todo lugar; *alternar* para alternar você para ela uma vez ao entrar em qualquer app que não tenha regra própria, depois deixá-lo livre (disparando de novo a cada novo app sem regra); ou **Nenhuma** para não fixar nada e agir como um alternador puro por app/por site. *Alternar* alterna você ativamente em todo app sem regra; **Nenhuma** não faz nada globalmente.
