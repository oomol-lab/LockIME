...
- **全域鎖定，或僅切換**——為全域預設輸入法設定三種行為之一：*鎖定*（lock）以在所有地方固定它；*切換*（switch）會在你進入任何沒有自己規則的應用程式時幫你切入一次，之後便放手讓你自由（每進入一個新的無規則應用程式就再觸發一次）；或者**無**（None）——什麼都不固定，把 LockIME 當作純粹的依應用程式／依站台切換器使用。*切換*會在每個無規則的應用程式主動幫你切入；**無**則在全域層級什麼也不做。
