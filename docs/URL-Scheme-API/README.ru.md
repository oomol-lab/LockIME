...
| `set-default-source` | `id` \| `name` *(omit both to clear)*, `action` = `lock` \| `switch` *(default `lock`)* | Задать (или сбросить) глобальный источник по умолчанию, не меняя состояние вкл/выкл. `action` определяет, будет ли значение по умолчанию **блокировать** (непрерывно применять источник) или **переключаться** на него один раз всякий раз, когда приложение возвращается к глобальному значению по умолчанию (когда ни одно правило для URL или приложения с более высоким приоритетом не закрепляет источник), а затем отпускать; на пути сброса он игнорируется. |
...
`bundle` также принимает синтетический идентификатор `process:<executable>`
процесса, у которого нет идентификатора bundle, потому что он запускается из
отдельного исполняемого файла, а не из `.app`, — например, `process:java` для
Minecraft, запущенного сторонним лаунчером. Это та же строка, которую для
такого процесса показывает встроенное окно выбора приложений.
...
| `list-log` *(aliases `log`, `recent-activations`)* | Записи журнала активаций за последние 24 ч, новейшие сначала: `{ "timestamp", "inputSource", "inputSourceName", "reason", "durationMs", "fromSourceName"?, "app"?, "bundleID"?, "ruleSource"?, "matchedHost"? }`. Строки с `"reason": "conflictPaused"` — диагностические (принудительное применение приостановлено после перетягивания с другим менеджером IME): переключение не выполнялось, а `durationMs` равно `0`. |
...
`defaultAction` (`lock` \| `switch`) сопровождает `defaultSource`, когда задан глобальный источник по умолчанию.
