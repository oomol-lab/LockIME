...
| `set-locking` *(alias `locking`)* | `enabled` = `true` \| `false` \| `toggle` | Включить/выключить **непрерывную блокировку** (или переключить её). Выкл ⇒ ничего не закрепляется, но правила переключения по-прежнему срабатывают. Не действует, пока главный переключатель выключен. |
