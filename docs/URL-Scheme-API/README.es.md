...
`bundle` también acepta la identidad sintética `process:<executable>` de un proceso que no
tiene identificador de bundle porque se ejecuta desde un ejecutable suelto en lugar de un
`.app` — p. ej. `process:java` para Minecraft iniciado por un launcher de terceros. Es la
misma cadena que el selector integrado en la aplicación muestra para un proceso así.
...
| `list-log` *(aliases `log`, `recent-activations`)* | Las últimas 24 h de entradas del registro de activación, las más recientes primero: `{ "timestamp", "inputSource", "inputSourceName", "reason", "durationMs", "fromSourceName"?, "app"?, "bundleID"?, "ruleSource"?, "matchedHost"? }`. Las filas con `"reason": "conflictPaused"` son de diagnóstico (el bloqueo quedó en pausa tras un tira y afloja con otro gestor de IME): no se realizó ningún cambio y `durationMs` es `0`. |
