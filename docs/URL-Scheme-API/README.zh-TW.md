...
| `set-default-source` | `id` \| `name` *(omit both to clear)*, `action` = `lock` \| `switch` *(default `lock`)* | 設定（或清除）全域預設輸入法，不改變開/關狀態。`action` 決定這個預設是要**鎖定**（持續強制使用該輸入法），還是在進入沒有自己規則的應用程式時**切換**到它一次，然後放手；在清除路徑上它會被忽略。 |
