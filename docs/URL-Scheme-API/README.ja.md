...
オン/オフの切り替え、入力ソースの再設定、ルールの管理、そして状態の読み取りが可能です。
...
`lock` / `unlock` / `toggle-lock` は **LockIME** をオン/オフします——これが
すべて（ロックと切り替えの両方）を制御する唯一のスイッチです。アプリごと / サイトごとの
切り替えルールを発火させ続けたまま、グローバルには何も固定しない——「純粋な切り替え役の
ように振る舞う」モードにするには、代わりにグローバルのデフォルトソースを**なし**に
設定してください（ソースを指定しない `set-default-source`）。

| Command | Parameters | Effect |
|---|---|---|
| `lock` | — | **LockIME** を**オン**にします——ルールを適用します。 |
| `unlock` | — | **LockIME** を**オフ**にします——完全にアイドル状態になります。 |
| `toggle-lock` *(alias `toggle`)* | — | LockIME のオン/オフを反転します。 |
...
| `set-default-source` | `id` \| `name` *(omit both to clear)*, `action` = `lock` \| `switch` *(default `lock`)* | オン/オフの状態を変えずに、グローバルのデフォルトソースを設定（またはクリア）します。`action` は、デフォルトがソースを**ロック**する（ソースを継続的に強制する）か、それとも、アプリがグローバルのデフォルトにフォールバックする（より優先度の高い URL やアプリのルールがソースを固定していない）たびに、一度だけそのソースへ**切り替えて**から解放するかを選びます。クリア処理の際には無視されます。 |
...
| `list-log` *(aliases `log`, `recent-activations`)* | 直近 24 時間のアクティベーションログのエントリ。新しいものから順に：`{ "timestamp", "inputSource", "inputSourceName", "reason", "durationMs", "fromSourceName"?, "app"?, "bundleID"?, "ruleSource"?, "matchedHost"? }`。`"reason": "conflictPaused"` の行は診断用です（他の IME 管理者との綱引きの末に強制を一時停止）——切り替えは行われておらず、`durationMs` は `0` です。 |
