...
| `list-log` *(aliases `log`, `recent-activations`)* | As últimas 24 h de entradas do registro de ativação, da mais recente para a mais antiga: `{ "timestamp", "inputSource", "inputSourceName", "reason", "durationMs", "fromSourceName"?, "app"?, "bundleID"?, "ruleSource"?, "matchedHost"? }`. As linhas com `"reason": "conflictPaused"` são de diagnóstico (aplicação pausada após um cabo de guerra com outro gerenciador de IME) — nenhuma troca foi realizada e `durationMs` é `0`. |
...
`defaultAction` (`lock` \| `switch`) acompanha `defaultSource` quando um padrão global está definido.
